...
module YamlHelper
  ...
  def self.to_hash_array(entities, include_lyric: true)
    list = Array(entities)
    list.map do |entity|
      data_hash = entity.to_attr_hash
      data_hash[:lyric] = entity.lyric if include_lyric && entity.respond_to?(:lyric) && entity.lyric
  ...
  def self.export_yaml(entities, file_path:, include_lyric: true)
    data = to_hash_array(entities, include_lyric: include_lyric)
